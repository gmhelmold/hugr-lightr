...
mod tests {
    ...
    #[test] fn invalidate_copy_changes_consumed_input_without_touching_source() { let source = temp("invalidate-source"); fs::write(source.join("Dockerfile"), "FROM scratch\nCOPY data.txt /data.txt\n").unwrap(); fs::write(source.join("data.txt"), "old\n").unwrap(); let copy = temp("invalidate-copy").join("fixture"); copy_tree(&source, &copy).unwrap(); let original = tree_sha256(&source).unwrap(); let mut s = scenario("invalidate", Availability::Supported); s.lightr.as_mut().unwrap().command = Some("$LIGHTR build $FIXTURE_DIR".into()); assert_eq!(invalidate_input(&s, &copy).unwrap(), PathBuf::from("data.txt")); fs::write(copy.join("data.txt"), "new\n").unwrap(); assert_eq!(tree_sha256(&source).unwrap(), original); assert_ne!(tree_sha256(&copy).unwrap(), original); assert!(!copy.join(".lightr-s2-invalidate").exists()); }
