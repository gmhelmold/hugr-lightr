...
fn run_differential(spec_path: &Path, chunk: usize, chunks: usize, rounds: usize, out: &Path, docker: &Path, lightr: &Path, mode: &str) -> Result<(), String> {
    ...
    for scenario in select_scenarios(&spec.scenarios, chunk, chunks) {
        ...
        for round in round_indices(rounds) {
            ...
            let state = context.map(|(fixture_dir, fixture_hash, _)| differential_mode_state(mode, scenario, docker, lightr, fixture_dir, fixture_hash, &scenario_out)).transpose()?;
...
fn differential_mode_state(mode: &str, scenario: &Scenario, docker: &Path, lightr: &Path, fixture: &Path, fixture_hash: &str, output: &Path) -> Result<ModeState, String> {
    ...
    let mutated = output.join("invalidate-fixture"); copy_tree(fixture, &mutated)?; let before = tree_sha256(&mutated)?; let mutation = b"lightr-s2-invalidate-v1\n"; fs::write(mutated.join(".lightr-s2-invalidate"), mutation).map_err(|e| format!("invalidate fixture mutation: {e}"))?; let after = tree_sha256(&mutated)?; if before == after || tree_sha256(fixture)? != fixture_hash { return Err(format!("invalidate mutation unverified for {}", scenario.id)); }
    Ok(ModeState { docker_command: None, fixture: mutated, fixture_hash: after, original_fixture_hash: fixture_hash.into(), mutation_sha256: sha256(mutation), mutation_receipt: "write:.lightr-s2-invalidate:sha256".into(), receipt: format!("docker_image_present:{tag};lightr_home_preserved;fixture_mutated") })
}
...
mod tests {
    ...
    #[test] fn invalidate_copy_changes_without_touching_source() { let source = temp("invalidate-source"); fs::write(source.join("Dockerfile"), "FROM scratch\n").unwrap(); let copy = temp("invalidate-copy").join("fixture"); copy_tree(&source, &copy).unwrap(); let original = tree_sha256(&source).unwrap(); fs::write(copy.join(".lightr-s2-invalidate"), b"lightr-s2-invalidate-v1\n").unwrap(); assert_eq!(tree_sha256(&source).unwrap(), original); assert_ne!(tree_sha256(&copy).unwrap(), original); }
    ...
    #[cfg(unix)] #[test] fn warm_lightr_home_survives_setup_to_timed_sample() { use std::os::unix::fs::PermissionsExt; let root = temp("warm"); let fixture = root.join("fixture"); fs::create_dir_all(&fixture).unwrap(); fs::write(fixture.join("Dockerfile"), "FROM scratch\n").unwrap(); let docker = root.join("docker"); fs::write(&docker, "#!/bin/sh\nexit 0\n").unwrap(); fs::set_permissions(&docker, fs::Permissions::from_mode(0o755)).unwrap(); let lightr = root.join("lightr"); fs::write(&lightr, "#!/bin/sh\nmkdir -p \"$LIGHTR_HOME\"\nif test -f \"$LIGHTR_HOME/setup\"; then touch \"$LIGHTR_HOME/timed\"; else touch \"$LIGHTR_HOME/setup\"; fi\n").unwrap(); fs::set_permissions(&lightr, fs::Permissions::from_mode(0o755)).unwrap(); let mut s = scenario("warm", Availability::Supported); s.docker.as_mut().unwrap().command = Some("$DOCKER build --tag warm:test $FIXTURE_DIR".into()); s.lightr.as_mut().unwrap().command = Some("$LIGHTR build $FIXTURE_DIR".into()); let output = root.join("out"); let state = differential_mode_state("warm", &s, &docker, &lightr, &fixture, &tree_sha256(&fixture).unwrap(), &output).unwrap(); let versions = Versions { docker_client: Some(DOCKER_VERSION.into()), docker_server: Some(DOCKER_VERSION.into()), docker_api: Some("1.51".into()), lightr: Some("test".into()), lightr_hash: Some("hash".into()), error: None }; let (_, timed) = execute_record(&s, "lightr", 0, Some((&state.fixture, &state.fixture_hash, "commit")), &output, &versions, "spec", &docker, &lightr, None, None); assert!(command_passed(&timed)); assert!(output.join("lightr-home/timed").is_file()); assert!(state.receipt.contains("docker_image_present:warm:test")); }
}
