...
class AnchoredScratchPolicyTests(unittest.TestCase):
    ...
    def test_composition_keeps_descendant_and_scratch_controls_and_suites(self):
        ...
        self.assertCountEqual(names, ["CASES", "EMPTY_CASES", "DESCENDANT_CASES", "SCRATCH_CASES", "LINK_CASES", "LISTING_CASES", "DEST_ANCHOR_CASES", "DEST_REPR_CASES", "DEST_PREPARE_CASES"])
        self.assertIn('descendant_expected = rf"test result: ok\\. {13 if linux else 12} passed;', text)
        self.assertIn('scratch_expected = r"test result: ok\\. 15 passed;', text)
        for family in ("descendant", "scratch"):
