...
fn ready_matches_frozen_si00_golden_bytes() {
    let values: Vec<serde_json::Value> =
        serde_json::from_str(include_str!("ready-golden.json")).unwrap();
    ...
    for value in values {
        ...
        assert_eq!(
            ...
            value["body"]["digest"].as_str().unwrap()
        );
        assert_eq!(receipt.length(), value["body"]["length"].as_u64().unwrap());
        for end in 0..raw.len() {
            assert!(
                Readiness::read(&mut &raw[..end]).is_err(),
...
fn readiness_rejects_resigned_semantic_corruption() {
    let normal = format!(
        r#"{{"version":1,"digest":"{}","length":1,"assurance":"unix-file-directory-v1"}}"#,
        "11".repeat(32)
    );
    for body in [
        normal.replace("\"version\":1", "\"version\":true"),
        ...
        normal.replace("11", "FF"),
        normal.replace("unix-file-directory-v1", "powerloss"),
    ] {
        assert!(
            Readiness::read(&mut resigned(body.as_bytes()).as_slice()).is_err(),
            "accepted {body}"
        );
    }
    assert!(Readiness::read(&mut resigned(b"\xff").as_slice()).is_err());
    let mut full = normal.into_bytes();
    full.resize(4096, b' ');
...
fn oversized_header_is_rejected_before_reading_body() {
    ...
    impl Read for HeaderOnly {
        fn read(&mut self, bytes: &mut [u8]) -> io::Result<usize> {
            self.reads += 1;
            assert!(
                self.header.position() < 12,
                "attempt to read overlength body"
            );
            self.header.read(bytes)
        }
    }
    let mut header = b"\0\0LSIR01".to_vec();
    header.extend_from_slice(&4097u32.to_le_bytes());
    let mut reader = HeaderOnly {
        header: io::Cursor::new(header),
        reads: 0,
    };
    assert!(Readiness::read(&mut reader).is_err());
...
fn streaming_hash_matches_bytes_and_preserves_reader_failure() {
    for length in [0, 1, 63, 64, 65535, 65536, 65537, 200000] {
        ...
        assert_eq!(
            Digest::of_reader(&mut data.as_slice()).unwrap(),
            (Digest::of_bytes(&data), length as u64)
        );
    }
    struct Broken;
    ...
    assert_eq!(
        Digest::of_reader(&mut Broken).unwrap_err().raw_os_error(),
        Some(5)
    );
    ...
    assert_eq!(
        Digest::of_reader(&mut Interrupted(true)).unwrap().0,
        Digest::of_bytes(b"")
    );
}
...
fn structured_failure_survives_legacy_wrapper_and_source_chain() {
    use std::error::Error;
    let original = PublicationFailure::new(
        [7; 16],
