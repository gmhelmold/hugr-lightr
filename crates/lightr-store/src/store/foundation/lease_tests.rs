...
fn lease_shared_excludes_gc_and_preserves_lock_file() {
    ...
    assert_eq!(
        domain.shared(Wait::Try).unwrap_err().kind(),
        io::ErrorKind::WouldBlock
    );
    drop(exclusive);
...
fn lease_cancellation_after_observed_contention_releases_waiter() {
    ...
    let error =
        super::lease_io::NativeLock::acquire_observed(&directory, ".gc.lock", false, wait, || {
            observed = true;
            cancellation.cancel();
        })
        .unwrap_err();
    assert!(
        observed,
        "test must observe the native WouldBlock before cancelling"
    );
...
fn lease_store_aliases_share_identity_and_domain() {
    ...
    assert_eq!(
        alias.exclusive(Wait::Try).unwrap_err().kind(),
        io::ErrorKind::WouldBlock
    );
}
...
fn lease_foreign_store_exclusive_is_not_shared_cache_exclusion() {
    ...
    assert_eq!(
        cache_a.exclusive(Wait::Try).unwrap_err().kind(),
        io::ErrorKind::WouldBlock
    );
    drop(cache_guard);
    let standalone = cache_a.exclusive(Wait::Try).unwrap();
    assert_eq!(
        worker.cache(&cache_b, Wait::Try).unwrap_err().kind(),
        io::ErrorKind::WouldBlock
    );
    drop(standalone);
    drop(foreign_gc);
...
fn lease_distinct_digest_workers_progress_independently() {
    ...
    assert_eq!(
        two.digests(&[a], Wait::Try).unwrap_err().kind(),
        io::ErrorKind::WouldBlock
    );
    drop(held);
...
fn lease_staging_borrows_guard_and_keeps_source_and_sentinel() {
    ...
    let mut staged = LeasedStagedFile::copy(
        &lease,
...
fn lease_missing_or_nondirectory_roots_do_not_get_created() {
    ...
    assert_eq!(
        StoreLocks::open_existing(&absent).unwrap_err().kind(),
