...
use crate::store::cas::owned_temp::OwnedTemp;
use std::{
...
pub fn install_metadata(
    ...
    bytes: &[u8],
    required: Assurance,
) -> Result<MetadataInstall, InstallFailure> {
...
pub(super) fn install_with(
        ...
        || name.len() > 128
    {
        return Err(failure(
            ...
            io::Error::new(
                ...
                "metadata name must be one bounded ASCII component",
            ),
        ));
    }
    let destination = parent.join(name);
    let fail =
        |stage, visibility, error| InstallFailure::new(stage, visibility, &destination, error);
    let assurance = Assurance::native();
    if assurance != required {
        return Err(fail(
            ...
            io::Error::new(
                io::ErrorKind::Unsupported,
                "requested synchronization profile is unavailable",
            ),
        ));
    }
    if bytes.len() > MAX_METADATA_BYTES {
        return Err(fail(
            ...
            io::Error::new(
                io::ErrorKind::InvalidInput,
                "metadata exceeds bounded protocol limit",
            ),
        ));
    }
    let metadata =
        fs::symlink_metadata(parent).map_err(|e| fail(Stage::Preflight, Visible::Unchanged, e))?;
    if !metadata.is_dir() || metadata.file_type().is_symlink() {
        return Err(fail(
            ...
            io::Error::new(
                io::ErrorKind::InvalidInput,
                "metadata parent must be a private real directory",
            ),
        ));
    }
    ...
    let staging = OwnedTemp::new(parent, "si01-metadata")
        .map_err(|e| fail(Stage::Allocate, Visible::Unchanged, e))?;
    ...
    file.write_all(bytes)
        .map_err(|e| fail(Stage::Write, Visible::Unchanged, e))?;
    ops.sync_file(&file)
        .map_err(|e| fail(Stage::SyncFile, Visible::Unchanged, e))?;
    ...
    loop {
        ...
        if offset
            ...
            || bytes[offset..offset + count] != buffer[..count]
        {
            return Err(fail(
                Stage::Verify,
                Visible::Unchanged,
                io::Error::new(
                    ...
                    "staged metadata differs from supplied bytes",
                ),
            ));
        }
        offset += count;
    ...
    staging
        .finish()
        .map_err(|e| fail(Stage::Cleanup, Visible::Confirmed, e))?;
    ops.sync_directory(parent)
        .map_err(|e| fail(Stage::Cleanup, Visible::Confirmed, e))?;
    Ok(MetadataInstall {
