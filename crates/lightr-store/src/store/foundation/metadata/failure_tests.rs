...
fn legacy_error_retains_downcastable_phase_and_original_os_cause() {
    let error = InstallFailure::new(
        InstallStage::SyncDirectory,
        InstallVisibility::InstalledUnconfirmed,
        Path::new("objects-ready/aa/bb"),
        io::Error::from_raw_os_error(28),
    );
    let LightrError::Io(outer) = error.into_legacy() else {
        panic!("changed frozen variant")
    };
    let payload = outer
        .get_ref()
        .unwrap()
        .downcast_ref::<InstallFailure>()
        .unwrap();
