...
use super::{
    install_metadata, Assurance, InstallStage as Stage, InstallVisibility as Visible,
    MAX_METADATA_BYTES,
};
use std::{
    fs::{self, File},
...
impl InstallIo for FaultIo {
    ...
    fn rename(&self, source: &Path, destination: &Path) -> io::Result<()> {
        if matches!(self.0, Fault::RenameBefore) {
            return Err(injected());
        }
        if matches!(self.0, Fault::ExtraChild) {
            fs::write(source.parent().unwrap().join("unknown"), b"retain")?;
        }
        NativeIo.rename(source, destination)?;
        if matches!(self.0, Fault::RenameAfter) {
            return Err(injected());
        }
        Ok(())
    }
    fn sync_directory(&self, parent: &Path) -> io::Result<()> {
        if matches!(self.0, Fault::Directory) {
            return Err(injected());
        }
...
fn checked_metadata_flush_failure_preserves_prior_destination() {
    let dir = TempDir::new().unwrap();
    fs::write(dir.path().join("entry"), b"old").unwrap();
...
fn cleanup_error_retains_confirmation_and_unknown_child() {
    ...
    assert_eq!(
        fs::read(leftovers[0].path().join("unknown")).unwrap(),
        b"retain"
    );
}
...
fn preflight_rejects_names_sizes_and_profile_without_writes() {
    let dir = TempDir::new().unwrap();
    for name in [
        "", "..", "../entry", "a/b", "a\\b", "a:b", "/entry", "C:entry",
    ] {
        assert_eq!(
            install_metadata(dir.path(), name, b"x", Assurance::native())
                .unwrap_err()
                .stage(),
            Stage::Preflight
        );
    ...
    let other = if cfg!(unix) {
        Assurance::WindowsFile
    } else {
        Assurance::UnixFileDirectory
    };
    assert_eq!(
        install_metadata(dir.path(), "entry", b"x", other)
            .unwrap_err()
            .io_error()
            .kind(),
...
fn missing_parent_and_directory_destination_are_not_repaired() {
    ...
    assert!(install_metadata(dir.path(), "entry", b"x", Assurance::native()).is_err());
...
fn bounded_large_metadata_roundtrips_without_payload_sized_readback() {
    let dir = TempDir::new().unwrap();
    let bytes = vec![0x5a; MAX_METADATA_BYTES];
    install_metadata(dir.path(), "entry", &bytes, Assurance::native()).unwrap();
    assert_eq!(fs::read(dir.path().join("entry")).unwrap(), bytes);
}
...
fn parallel_metadata_installs_use_distinct_owned_staging() {
    ...
    for byte in 0..8u8 {
        assert_eq!(
            fs::read(dir.path().join(format!("entry{byte}"))).unwrap(),
            vec![byte; 1024]
        );
    }
}
...
fn metadata_rejects_symlink_destination_and_preserves_target() {
    let dir = TempDir::new().unwrap();
    fs::write(dir.path().join("target"), b"keep").unwrap();
    std::os::unix::fs::symlink("target", dir.path().join("entry")).unwrap();
    assert!(install_metadata(dir.path(), "entry", b"new", Assurance::native()).is_err());
